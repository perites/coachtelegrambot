...
class CoachHandler:
    ...
    def see_my_session(self, message):
        if not get_coach_by_username(message.from_user.username):
    ...
    @staticmethod
    def _see_my_session_parts():

        text = "Виберіть тип сесій: "
        markup = types.InlineKeyboardMarkup(row_width=1)
        markup.add(
            types.InlineKeyboardButton("Індивідуальний формат", callback_data="coach;show;single"),
            types.InlineKeyboardButton("Груповий формат", callback_data='coach;show;group')
        )
        return text, markup
...
class CoachCallbackHandler(CoachHandler):
    ...
    def show_callback_handler(self, call):
        ...
        markup.add(types.InlineKeyboardButton("Назад", callback_data=f'coach;back_to_types'))
